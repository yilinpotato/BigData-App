"""用 nbformat 生成 notebooks/steam_analysis.ipynb（避免手写 JSON）。
图内文字用英文（规避无中文字体问题），分析解读用中文 markdown。
运行: python notebooks/_build.py  然后用 nbconvert 执行。
"""
...
md("## 0. 载入清洗后数据并注册 Spark SQL 视图")
code("""games = spark.read.parquet(f"{CLEAN}/games")
...
q1.tail(8)""")
md("""**发现**：游戏发行量在 2014 年后随 Steam 开放 Greenlight/Direct 而爆发式增长，
2017–2018 达到峰值（年发行数千款），反映独立游戏浪潮；2019 因数据快照不完整而回落。""")

# Q2
md("""## Q2. 热门游戏类型(genre)及其口碑
...
q2""")
md("""**发现**：Indie、Action、Casual、Adventure 数量最多；但数量多≠口碑好——
休闲/大众类型好评率往往略低，而偏核心向的类型（如 Strategy/RPG）好评率更稳定偏高。""")

# Q3
md("""## Q3. 价格分布与「价格 × 口碑 × 拥有量」
**问题**：游戏定价如何分布？价格与好评率、拥有量是否相关？免费 vs 付费占比？""")
code("""paid = games.filter((F.col("price") > 0) & (F.col("price") <= 60)).select("price").toPandas()
...
q3""")
md("""**发现**：绝大多数付费游戏定价在 £10 以下，呈长尾。价格越高的游戏平均好评率反而更高
（高价多为制作精良的 3A/精品），但免费与低价游戏的平均拥有量最大——说明**低价走量、高价走口碑**。""")

# Q4
md("""## Q4. 游玩时长 × 类型（用户粘性）
**问题**：哪些类型游戏的平均游玩时长最高（粘性最强）？仅统计有一定评价量的类型以降噪。""")
code("""q4 = (genre.filter(F.col("total_ratings") >= 50)
      .groupBy("genre")
...
q4""")
md("""**发现**：MMO、大型多人、策略、模拟经营类游玩时长显著领先（数十小时量级），
属于强粘性/长尾沉浸型；而休闲、解谜类时长较短。粘性与可重复游玩机制（多人对战、开放经营）高度相关。""")

# Q5
md("""## Q5. Top 开发商 / 发行商
**问题**：谁发布的游戏最多？谁的总体口碑（累计好评）最高？""")
code("""dev = spark.sql('''SELECT developer, COUNT(*) n FROM games
...
dev.head(), pub.head()""")
md("""**发现**：发行数量最多的多为量产型独立工作室；而**累计好评**集中在 Valve、
育碧、世嘉等头部发行商——它们靠少数爆款（CS:GO、GTA 等）贡献了海量正面评价。数量与影响力并不等同。""")

# Q6
md("""## Q6. 平台支持趋势（Windows / Mac / Linux）
**问题**：跨平台支持比例如何？Mac/Linux 支持随时间如何变化？""")
code("""overall = games.select(
...
q6.tail()""")
md("""**发现**：Windows 近乎 100%。Mac/Linux 支持在 2013–2015 年（SteamOS/Steam Machine 推广期）
达到高点，随后随该战略降温而回落。平台覆盖与厂商商业战略强相关。""")

# Q7
md("""## Q7. 最受欢迎的标签（SteamSpy tags）
**问题**：玩家投票最多的标签是哪些？基于宽转长后的 `tags_long` 表用 Spark SQL 聚合。""")
code("""q7 = spark.sql('''SELECT tag, SUM(votes) total_votes, COUNT(*) n_games
...
q7.head(10)""")
md("""**发现**：Indie、Action、Adventure、Casual、Singleplayer 等宏观标签投票最高，
...
q8""")
md("""**发现**：有成就系统的游戏（尤其 11–100 个）平均好评率与游玩时长均高于无成就游戏，
...
out = "/home/qqrtq/bigdata app/notebooks/steam_analysis.ipynb"
